...
export async function action({ request }: Route.ActionArgs) {
  ...
  // Parse locale mapping (format: "English:en,Indonesian:id")
  const localeMapping: Record<string, string> = {};
  const mappingStr = formData.get("localeMapping") as string;
  if (mappingStr) {
    const pairs = mappingStr.split(",");
    for (const pair of pairs) {
      const [figmaMode, existingLocale] = pair.split(":").map(s => s.trim());
      if (figmaMode && existingLocale) {
        localeMapping[figmaMode] = existingLocale;
      }
    }
  }

  // Parse existing ARB/JSON files for merging
  const existingFiles: Record<string, Record<string, string>> = {};
  const uploadedFiles = formData.getAll("existingFiles") as File[];
  for (const file of uploadedFiles) {
    if (file && file.size > 0) {
      try {
        ...
        const locale = file.name.replace(/\.(arb|json)$/, "");

        const entries = Object.entries(data)
          .filter(([k, v]) => !k.startsWith("@@") && typeof v === "string") as [string, string][];
        existingFiles[locale] = Object.fromEntries(entries);
      } catch {
        // Skip invalid files
      }
    }
  ...
  // Build reverse mapping: Figma mode -> existing locale
  const modeToExistingLocale: Record<string, string> = {};
  for (const [figmaMode, existingLocale] of Object.entries(localeMapping)) {
    modeToExistingLocale[figmaMode] = existingLocale;
  }

  function mergeWithExisting(strings: Record<string, string>, figmaMode: string): Record<string, string> {
    const existingLocale = modeToExistingLocale[figmaMode] || figmaMode;
    const existing = existingFiles[existingLocale] || {};
    ...
    try {
      ...
      if (json.collections && Array.isArray(json.collections)) {
        ...
        for (const [mode, strings] of Object.entries(result.jsonGenerated)) {
          ...
          const merged = mergeWithExisting(strings, localeInfo);
          files.push({
            name: `${localeInfo}.json`,
            content: JSON.stringify(merged, null, 2),
          });
          // Also generate ARB if merging with existing
          if (existingFiles[localeInfo]) {
            const arbContent = {
              ...Object.fromEntries(
            ...
            files.push({
              name: `app_${localeInfo}.arb`,
              content: JSON.stringify(arbContent, null, 2),
            });
          }
        }
      ...
      function extractKeys(obj: Record<string, unknown>, prefix = "") {
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (typeof value === "string") {
            ...
            extractKeys(obj as Record<string, unknown>, newKey);
          }
        }
      }
      ...
      const merged = mergeWithExisting(jsonGenerated, "locale");
      return {
        success: true,
        files: [
          { name: "locale.json", content: JSON.stringify(merged, null, 2) },
        ],
      };
    } catch {
          ...
          <div className="p-6">
            {mode === "upload" && (
              ...
              >
                ...
                <div>
                  <label
                    htmlFor="existingFiles"
                    className="block text-sm font-medium text-[#191c1e] mb-2"
                  >
                    Existing ARB/JSON Files (optional)
                  </label>
                  <input
                    type="file"
                    id="existingFiles"
                  ...
                  />
                  <p className="mt-1 text-xs text-[#64748b]">
                    Upload existing ARB/JSON files to merge (new values override)
                  </p>
                </div>

                <div>
                  <label
                    htmlFor="localeMapping"
                    className="block text-sm font-medium text-[#191c1e] mb-2"
                  >
                  ...
                  <input
                    ...
                    id="localeMapping"
                    name="localeMapping"
                    placeholder="English:en,Indonesian:id"
                    className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg text-sm text-[#191c1e]
                               placeholder:text-[#94A3B8]
                               focus:outline-none focus:ring-2 focus:ring-[#2563EB]/40 focus:border-[#2563EB]"
                  />
                  <p className="mt-1 text-xs text-[#64748b]">
                    Map Figma modes to existing file locales (format: Mode:Filename, Mode:Filename)
                  </p>
                </div>
